...
class ResBlock1(torch.nn.Module):
    def __init__(self, channels, kernel_size=3, dilation=(1, 3, 5)):
        ...
        self.convs1 = nn.ModuleList([
            weight_norm(Conv1d(
                channels, channels,
                kernel_size, 1, dilation=dilation[0],
                padding=get_padding(kernel_size, dilation[0]))),
            weight_norm(Conv1d(
                channels, channels,
                kernel_size, 1, dilation=dilation[1],
                padding=get_padding(kernel_size, dilation[1]))),
            weight_norm(Conv1d(
                channels, channels,
                kernel_size, 1, dilation=dilation[2],
...
class Generator(torch.nn.Module):
    def __init__(self, hparams: DictConfig):
        ...
        self.conv_pre = weight_norm(Conv1d(80, hparams.upsample_initial_channel, 7, 1, padding=3))
        ...
        for i, (u, k) in enumerate(zip(hparams.upsample_rates, hparams.upsample_kernel_sizes)):
            self.ups.append(weight_norm(
                ConvTranspose1d(hparams.upsample_initial_channel//(2**i), hparams.upsample_initial_channel//(2**(i+1)),
                                k, u, padding=(k-u)//2)))
...
class DiscriminatorP(torch.nn.Module):
    def __init__(self, period, kernel_size=5, stride=3, use_spectral_norm=False):
        ...
        self.convs = nn.ModuleList([
            ...
            norm_f(Conv2d(
                32, 128, (kernel_size, 1),
                (stride, 1), padding=(get_padding(5, 1), 0))),
            norm_f(Conv2d(
                128, 512, (kernel_size, 1),
                (stride, 1), padding=(get_padding(5, 1), 0))),
            norm_f(Conv2d(
                512, 1024, (kernel_size, 1),
                (stride, 1), padding=(get_padding(5, 1), 0))),
            norm_f(Conv2d(
